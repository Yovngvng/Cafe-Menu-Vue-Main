...
| مالیات | ۱۰٪ برای جمع جزء بالای ۳۰۰ هزار تومان |
| QR میز | لینک مستقیم به میز سالن یا فضای باز |
| ادمین | ورود با Supabase Auth، سفارش زنده، فیلتر وضعیت، بوق سفارش جدید، موجودی روزانه، قیمت‌ها |
...
دو میانبر جدا بسازید:

| میانبر | آدرس | نام |
|---|---|---|
| منوی مشتری | صفحه اصلی سایت | کافه ژوان |
| پنل ادمین | `/admin` | مدیریت کافه |

روی آیفون از سافاری «افزودن به صفحه اصلی» را **همان صفحه‌ای که می‌خواهید** باز کنید. اعلان سفارش روی iOS معمولاً فقط وقتی کار می‌کند که پنل از میانبر نصب‌شده باز شده باشد. اگر تب کاملاً بسته شود، اعلان پس‌زمینه به Web Push سمت سرور نیاز دارد.

ورود با «مرا به خاطر بسپار» را برای آشپزخانه روشن نگه دارید تا نشست با بستن مرورگر پاک نشود.
...
4. `supabase/03_item_availability.sql` — موجودی روزانه آبمیوه و کیک
5. `supabase/04_menu_price_overrides.sql` — قیمت‌های قابل ویرایش ادمین
